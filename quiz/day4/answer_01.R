...
# 6. df 객체의 holiday 변수가 1이면서, 
#    workingday 변수가 1인 경우가 존재하는지 확인하시오.
# 1) 
...
# 7. df 객체의 weather 변수에는 고유값(중복제거된값)이
# 몇 개 있는지 기술하시오
...
# 10. df 객체의 count 변수의 상위 1% 값 보다 큰 값을
# 이상치로 간주하고자 한다. 상위 1% 값을 구하시오.
quantile(df$count, probs = 0.99)

# 11. df 객체의 짝수 번째 row만 추출하여 
# "bike_even.csv" 파일로 저장하시오
write.csv(df[seq(from = 2, to = nrow(df), by = 2), ],
          "bike_even.csv", row.names = FALSE)
